...
class T5ForPromptDSTConfig(T5Config):
    def __init__(
        ...
        self.num_graph_prompt_tokens = len(GRAPH_PROMPT_TOKENS)
...
class T5ForPromptDST(T5ForConditionalGeneration):
    ...
    def __init__(self, config:T5ForPromptDSTConfig):
        ...
        self.graph_prompt_embedder = nn.Embedding(config.num_graph_prompt_tokens, config.d_model)
        self.graph_prompt_bias = nn.Embedding(config.num_graph_prompt_tokens, config.num_heads, _weight=torch.zeros(config.num_graph_prompt_tokens,config.num_heads))
        ...
        self.init_weights() #T5PreTrainedModel _init_weights(), no need for checkpoint
    ...
    def convert_input_ids_to_input_embeds(self, input_ids, slot_connect, ds_ids, ds_mask):
        ...
        inputs_embeds = torch.where((input_ids < self.vocab_size + self.config.num_prompt_tokens + self.config.num_graph_prompt_tokens).unsqueeze(-1), inputs_embeds, meta_prompt_embeds)
    ...
    def prepare_pos_attn_mask_for_encoder_prompt(self, input_ids):
        ...
        graph_prompt_ids = input_ids - self.vocab_size - self.config.num_prompt_tokens
        graph_prompt_mask = graph_prompt_ids >= 0
        meta_prompt_ids = input_ids - self.vocab_size - self.config.num_prompt_tokens - self.config.num_graph_prompt_tokens
        ...
        graph_prompt_ids = torch.where(graph_prompt_mask, graph_prompt_ids, torch.zeros_like(graph_prompt_ids))
        graph_prompt_bias = self.graph_prompt_bias(graph_prompt_ids)
        graph_prompt_bias[~graph_prompt_mask] = 0

        bias = prompt_bias + meta_prompt_bias + graph_prompt_bias
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.convert_input_ids_to_input_embeds(input_ids, batch_graph)
        if decoder_inputs_embeds is None:
            decoder_inputs_embeds = self.convert_input_ids_to_input_embeds(decoder_input_ids, batch_graph)
...
class T5ForPromptEncDecDST(T5ForPromptDST):
    ...
    def prepare_inputs_for_generation(
        ...
        return {
            ...
            'decoder_inputs_embeds': self.convert_input_ids_to_input_embeds(input_ids, kwargs['batch_graph']),
