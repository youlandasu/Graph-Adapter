    ...
    for dial in data:
        ...
        for idx_t, t in enumerate(dial['dialogue']):
            ...
            if (t['spk'] == "USER"):
                ...
                slots = t["utt"].strip()
                dialogues.append({"history": " ".join(plain_history[-args.max_history:]), #-100
    ...
    for split in datasets.keys():
        for tasks_id, task in aggregate["BYDOMAIN"][split].items():
            if (args.task_type == "E2E"):
                ...
                datasets[split][tasks_id] = get_DST_from_dial(args, task, tasks_id, tokenizer, max_train_dials_per_domain, split) #GET DST FROM DIALS
