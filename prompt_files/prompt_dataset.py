...
class MemT5PromptDSTDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 num_graph_prompt=50, # graph_prompt
        ...
        for domain in self.domains:
            self.all_slots.extend(self.domain2slot[domain])
        ...
        graph_prompt = self.domain2graph_prompt()
        ds_sequence = self.domain2slot_token()
        ...
        if type_path in ['train', 'val']:
            ...
            for domain, dials in self.dialogs.items():
                ...
                for dial in dials:
                    self.domain2samples[domain].append(self.convert_dial_to_example(dial, domain, soft_prompt, graph_prompt, ds_sequence)) # graph_prompt
            if small_sample_run:
    ...
    def convert_dial_to_example(self, dial, domain, soft_prompt, graph_prompt, ds_sequence, do_augment=False, extra_aug_slots=[]):# graph_prompt
        ...
        input_dict = {
            'dst_input_sequence': (dial['history'] + ' </s> ' + graph_prompt + domain_enc_prompt + soft_prompt).lower(), ## graph_prompt
    ...
    def resample_dataset(self, cur_domain, training_prompt_name, forward_domains:List=None, backward_domain=None,
        ...
        # Convert and append domain dials to example
        for domain, dials in self.dialogs.items():
        ...
        # Convert and append memory_replay dials to example
    ...
    def collate_fn(self, batch):
        ...
        dst_ds_ids = self.tokenizer(dst_ds_seqs,
                                        max_length=self.num_graph_prompt, #same as prompts_config
                                        padding=True,
                                        truncation=True,
                                        return_tensors='pt')
        dst_ds_ids = dst_ds_ids['input_ids']
        dst_ds_mask = dst_ds_ids['attention_mask']
        ...
        dst_ds_ids, dst_ds_mask = self.fill_with_mask(dst_input_ids, dst_ds_ids)
        print(dst_ds_ids)
        print(dst_ds_mask)
        #raise Exception('check input ds_ids')
        graph_list = []
        for i in batch:
            graph_list.append(self.convert_ontology_to_graph(dst_ds_ids[i], dst_ds_mask[i], slot_connect[i]))
        batch_graph = Batch.from_data_list(graph_list) #.to(feats.device)
    ...
    def domain2graph_prompt(self, domain=None): #meta graph prompt
        for slot in self.all_slots:
            #graph_prompt = ''.join([s.split(':')[1] for s in self.slot2description[slot]])
            graph_prompt = ''.join(['<meta_prompt_{}>'.format(i) for i in range(self.slot2description)])
        return graph_prompt

    # construct input tokens to GCN: <extra_id_{slot_0}> ... <extra_id_{slot_n}>
    def domain2slot_token(self, domain=None): #meta graph prompt
        for slot in self.all_slots:
            ds_sequence = ''.join([s.split(':')[1] for s in self.slot2description[slot]])
        return ds_sequence

    def fill_with_mask(self, dst_input_ids, ds_ids, ds_mask=None):
        dst_ds_ids = np.zero_like(np.array(dst_input_ids))
        dst_ds_mask = np.zero_like(np.array(dst_input_ids))
        graph_prompt_ids = np.array(dst_input_ids) - self.vocab_size - self.config.num_prompt_tokens
        dst_ds_ids[np.where(graph_prompt_ids < self.num_graph_prompt) & (graph_prompt_ids >= 0)] = ds_ids
        dst_ds_mask[np.where(dst_ds_ids > 0)].fill(1)
        return dst_ds_ids, dst_ds_mask


    def convert_ontology_to_graph(self, ds_ids, ds_mask, slot_connect):
        
        #device = ontology.device
        ds_slot_connect = np.zeros((len(ds_mask), len(ds_mask)))
        ds_slot_connect[:,:] = ds_mask[np.newaxis,:] > 0
        ds_slot_connect[np.where(ds_slot_connect > 0)] = np.array(slot_connect)


        edge_index = self.get_edge_index_from_adj_matrix(torch.from_numpy(ds_slot_connect))
        y = strategies_vec[i+1, :-1].reshape(1, -1)	
        graph_data = Data(x=torch.from_numpy(ds_ids).unsqueeze(-1), edge_index=edge_index, y=y)
        return graph_data
    ...
    def prepare_for_generation(self, dial, domain):
        return self.convert_dial_to_example(dial, domain, self.domain2soft_prompt(domain), self.domain2graph_prompt(domain))#graph_prompt
...
class T5PromptGenFullStateDataset(MemT5PromptDSTDataset):
    ...
    def convert_dial_to_example(self, dial, domain, soft_prompt, graph_prompt, ds_sequence, do_augment=False, extra_aug_slots=[]):
        input_dict = {
    ...
    def collate_fn(self, batch):
        ...
        dst_ds_ids = dst_ds_ids['input_ids']
        dst_ds_mask = dst_ds_ids['attention_mask']
        batch_graph = self.convert_ontology_to_graph(dst_ds_ids, dst_ds_mask, slot_connect)
